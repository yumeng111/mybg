...
def addAllVariables(dfw, syst_name, isData, trigger_class, lepton_legs, isSignal, global_params):
    #yumeng:
        ...
        #dfw.Apply(AnaBaseline.GenSelection)
    print(f"Adding variables for {syst_name}")
    dfw.Apply(CommonBaseline.SelectRecoP4, syst_name, global_params["nano_version"])
    #yumeng:
    dfw.Apply(AnaBaseline.GenSelection, global_params)
    
    #yumeng:
    dfw.Apply(AnaBaseline.GenJetSelection)
    print("Yumengdebug")
    ...
    if global_params["my_channel"]== "sle":
        dfw.df=dfw.df.Filter("Electron_pt[Electron_selp4].size()>=1", "e_pt10")
        dfw.df=dfw.df.Filter("lep1_type==0 && lep2_type==-1", "SLe")
    ...
    #yumeng:
    if global_params["my_channel"]== "slmuon":
        dfw.df=dfw.df.Filter("lep1_genLep_kind==2||lep1_genLep_kind==4", "Mu_genMu")

    if global_params["my_channel"]== "sle":
        dfw.df=dfw.df.Filter("lep1_genLep_kind==1||lep1_genLep_kind==3", "e_genElectron")