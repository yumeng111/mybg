...
def createAnatuple(inFile, treeName, outDir, setup, sample_name, anaCache, snapshotOptions,range, evtIds,
    ...
    for index,fileName in enumerate(outfilesNames):
        ...
        hist_pass, hist_eff = ReportTools.SaveReport(reports[index].GetValue(), reportName=f"Report")
        outputRootFile.WriteTObject(hist_pass, f"Report_Number", "Overwrite")
        outputRootFile.WriteTObject(hist_eff, f"Report_Efficiency", "Overwrite")
        if index==0:
        ...
        #     report.Print()
    #yumeng:
    #print("global_params:", setup.global_params)
