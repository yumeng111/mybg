                             ...
                             const ROOT::VecOps::RVec<RVecI>& GenPart_daughters, std::ostream& os)
{
  ...
  for(int d_idx = 0; d_idx < daughters.size(); ++d_idx) {
    ...
    //yumeng
    const int d_pdgId = GenPart_pdgId[n];
    if (std::abs(GenPart_pdgId[genPart_idx]) == 24){
            if (std::abs(d_pdgId) == 11 || std::abs(d_pdgId) == 13) {
                if (GenPart_pt[n] < 5.0) {
                    os << pre << "+-> ";
                    const char pre_first = d_idx == daughters.size() - 1 ? ' ' : '|';
                    const std::string pre_d = pre + pre_first;
                    PrintDecayChainParticle(evt, n, GenPart_pdgId, GenPart_genPartIdxMother, GenPart_statusFlags,
                                            GenPart_pt, GenPart_eta, GenPart_phi, GenPart_mass, GenPart_status, pre_d,
                                            GenPart_daughters, os);
                }
            }
    } else {
            os << pre << "+-> ";
