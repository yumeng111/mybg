...
def genDistribution_hist(mass_point,df):
    if var == "pt":
        ...
        df= df.Define("NuoverNuPlusLepPtW","Nu_p4W.pt()/(Nu_p4W.pt()+GenLep_p4W.pt())")
        ...
        # hist_pt = df.Histo1D(model, "NuoverNuPlusLepPtW")
        # hist_pt.GetXaxis().SetTitle(f"pT(Nu)/pT(Nu)+pT(Lep) (WFrame,{mychannel} channel)")
        # hist_pt.GetXaxis().SetRangeUser(0, 1)
        
        # hist_pt = df.Histo1D(model, "VCandidateE")
        # hist_pt.GetXaxis().SetTitle(f"VCandidateE,{mychannel} channel)")
        # hist_pt.GetXaxis().SetRangeUser(0, 1500)
