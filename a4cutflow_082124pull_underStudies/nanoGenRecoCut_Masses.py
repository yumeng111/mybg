...
def SaveReport(report, initial_count, genSL_count,  gen_accept_count, skim_count, reportName="Cutflow", printOut=False):
    ...
    for i in range(1, hist_eff.GetNbinsX() + 1):
        ...
        latex.DrawLatex(bin_center, bin_content + 500, f"{bin_content:.2f}")

    canvas.SaveAs(f"plots/nanoGenRecoCut_{mychannel}/nanoGenRecoCutPass_{mychannel}{mass_point}.png")
    #canvas.SaveAs(f"plots/nanoGenRecoCutEff_{mychannel}/nanoGenRecoCut_{mychannel}{mass_point}.png")
