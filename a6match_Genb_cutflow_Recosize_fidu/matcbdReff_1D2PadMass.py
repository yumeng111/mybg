...
def Eff_RecoGenMatch_hist(mass_point,df):
    ...
    #variable changes
    #bins = int((pt_max - pt_min)/10)  
    bins = int((pt_max - pt_min)/50)  

    df=df.Define("RecoLepIdx", f"FindMatching(GenLep0_p4, {mychfull}_p4, 0.02)")
    df_matcdR=df.Filter("RecoLepIdx>=0 && nElectron>0", "")

    #variable changes, getvalue for entry
    h_before = df.Histo1D(("h_before", f"{particl}_Pt Distribution", bins, pt_min, pt_max), f"{particl}Pt").GetValue()
    h_after = df_matcdR.Histo1D(("h_after", f"Match_Eff vs {particl}_pt (M-{mass_point})", bins, pt_min, pt_max), f"{particl}Pt").GetValue()
    ...
    legend.Draw()
    h_before.GetXaxis().SetTitle(f"pT(GeV), {mychannel} Channel")
    ...
    c.SaveAs(f"plots/{mychbrief}{mass_point}{particl}_Eff_RecoGenMatch.png")
