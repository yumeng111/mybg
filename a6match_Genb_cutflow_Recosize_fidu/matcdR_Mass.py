...
def process_tree_Gen(file_path, tree_name):
    ...
    if tree_name == "Events":
        ...
        df = df.Define(f"{mychfull}_p4", f"ROOT::VecOps::RVec<LorentzVectorM> p4({mychfull}_pt.size()); \
                               for (int i = 0; i < {mychfull}_pt.size(); i++) {{ \
                                   p4[i] = LorentzVectorM(static_cast<double>({mychfull}_pt[i]), \
                                                          static_cast<double>({mychfull}_eta[i]), \
                                                          static_cast<double>({mychfull}_phi[i]), \
                                                          static_cast<double>({mychfull}_mass[i])); \
                               }} \
                               return p4;")
    ...
    #df= df.Define("GenJet0_p4","H_to_VV.legs.at(1).leg_p4.at(0)")
...
def RecoHWWCandidateSelection(df):
    ...
    df=df.Define("RecoLepIdx", f"FindMatching(GenLep0_p4,{mychfull}_p4, 0.2)")
    if mychannel == "sle":
...
def genDistribution_hist(mass_point,df):
    if var == "pt":
        ...
        # #for eventId
        # thresh="5"
        # df_filtered = df.Filter(f"GenLep0_pt > 0 && GenLep0_pt < {thresh}")

        # eventIds = df_filtered.AsNumpy(["event"])["event"]

        # num_eventIds = len(eventIds)
        # if num_eventIds > 0:
        #     print(f"Event IDs where 0 < pt < {thresh}:")
        #     print(eventIds[:min(20, num_eventIds)])
        # return "dummy_hist"
...
def genParticleDistribution_hist(mass_point,df):
    ...
    # #particle name consistent with df
    ...
    pt_min = 0
    # # pt_min = df.Min(f"{particle}_pt").GetValue()
    # # pt_max = df.Max(f"{particle}_pt").GetValue()
    # bins = int((pt_max - pt_min)/bin_wid)

    # model = ROOT.RDF.TH1DModel(f"{particle}_pT_M-{mass_point}", f"{particle} pT for Masspoints ({mychannel})", bins, pt_min, pt_max)
    # hist_pt = df.Histo1D(model, f"{particle}Pt")
    ...
    # hist_pt.GetXaxis().SetRangeUser(pt_min, pt_max)
    # hist_pt.GetYaxis().SetRangeUser(pt_min, pt_max)
...
def main(mychannel, var, hist_type):
    ...
    for i, mass_point in enumerate(mass_points):
        ...
        if hist_type =="genParticleDistribution":
            hist, yrange, particle=process_genParticleDistribution_hist(mass_point, df_GenSL_Eventstree, df_GenSL_Notselected)
        ...
        if hist_type =="cutflow":
                canvas.SaveAs(f"plots/MiniGenRecoCut_mass_{mychannel}.png")
        if hist_type =="genDistribution": 
                canvas.SaveAs(f"plots/cutplusGen_{var}_mass_{mychannel}.png")
        if hist_type =="genParticleDistribution": 
                canvas.SaveAs(f"plots/Gen{particle}_{var}{mass_point}_mass_{mychannel}.png")
        if hist_type == "mindR_GenLepJet":
                canvas.SaveAs(f"plots/mindR_GenLepJet_mass_{mychannel}.png")
...
######################################################################def of processing genDistribution hist -> only keep skimtree for eventid
def process_genDistribution_hist(mass_point, df_GenSL_Eventstree, df_GenSL_Notselected):
    # #if only for eventid:
    # hist = genDistribution_hist(mass_point,df_GenSL_Eventstree)
...
######################################################################def of processing genParticleDistribution hist -> 2D plots
def process_genParticleDistribution_hist(mass_point, df_GenSL_Eventstree, df_GenSL_Notselected):
    #if no need for adding Events and EventsNotSelected, only 2 lines below
    hist1, yrange, particle = genParticleDistribution_hist(mass_point,df_GenSL_Eventstree)
    hist = hist1.GetValue()  # Ensure the histogram is fully evaluated
    
    # hist1_evaluated = hist1.GetValue() 
    # hist2, _, _ = genParticleDistribution_hist(mass_point, df_GenSL_Notselected)
    # hist2_evaluated = hist2.GetValue()
    # hist = hist1_evaluated.Clone(f"HVVCandPt_add_{mass_point}")
    # hist.Add(hist2_evaluated)

    return hist, yrange, particle
...
hist_type ="genParticleDistribution"
