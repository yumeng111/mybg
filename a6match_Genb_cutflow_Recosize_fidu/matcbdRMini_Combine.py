...
def process_tree_Gen(file_path, tree_name):
    ...
    if tree_name == "Events":
        ...
        df=df.Define("FindMatchingdR",f"FindMatchingdR(H_to_VV.legs.at(0).leg_vis_p4.at(0), {mychfull}_p4, {dRthresh})")
...
def genDistribution_hist(mass_point,df):
    if var == "pt":
        ...
        model = ROOT.RDF.TH1DModel(f"recolepdRSize-{mass_point}", f"mindR(Recolep, Genlep))", bins, pt_min, dRthresh)
        ...
        hist_pt.GetXaxis().SetRangeUser(0, dRthresh)
...
def main(mychannel, var, hist_type):
    ...
    for i, mass_point in enumerate(mass_points):
        ...
        if integral != 0:
            ...
            for bin_idx in range(1, hist.GetNbinsX()):
        ...
        xcut=0.02 #0.4
        bin_dR = hist.FindBin(xcut)
        integral_above = hist.Integral(bin_dR + 1, hist.GetNbinsX())
        print(f"Fraction passing mindR > Bin({xcut}) for M-{mass_point}: {integral_above}")
        xcut1=0.2 #0.4
        bin_dR = hist.FindBin(xcut1)
        integral_above = hist.Integral(bin_dR + 1, hist.GetNbinsX())
        print(f"Fraction passing mindR > Bin({xcut1}) for M-{mass_point}: {integral_above}")
