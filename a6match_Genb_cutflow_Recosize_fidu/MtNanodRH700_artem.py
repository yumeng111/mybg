...
def EventsDistribution_hist(mass_point,df):  
    ...
    df = df.Define("dPhi_HVV_HBB", "ROOT::Math::VectorUtil::DeltaPhi(H_to_VV.cand_p4, H_to_BB.cand_p4)")
    #df = df.Filter("dPhi_HVV_HBB!=0")
    df = df.Define("HVVphi", "H_to_VV.cand_p4.Phi()")
    df = df.Define("HBBphi", "H_to_BB.cand_p4.Phi()")
    ...
    # hist_pt = df.Histo1D((f"dPhi_HVV_HBB, M{mass_point}", f"dPhi_HVV_HBB", 50, -4, 4), "dPhi_HVV_HBB")
    # hist_pt.GetXaxis().SetTitle(f"dPhi_HVV_HBB")
    # fname="dPhi_HVV_HBB" 

    hist_pt = df.Histo1D((f"HVV_phi, M{mass_point}", f"HVV_phi", 50, -4, 4), "HVVphi")
    hist_pt.GetXaxis().SetTitle(f"HVV_phi")
    fname="HVV_phi" 
...
def main(mychannel, var, hist_type):
    ...
    for i, mass_point in enumerate(mass_points):
        ...
        if  hist_type == "EventsDistribution":
            hist, yrange, fname= EventsDistribution_hist(mass_point, df_eventstree)
