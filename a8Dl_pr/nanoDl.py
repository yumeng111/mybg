...
def Report_cutflow_hist(canvas, mass_point, report, initial_count, genDL_count,  gen_accept_count, genb_accept_count, skim_count, reportName="Cutflow", printOut=False):
    ...
    if ctype == "Relative":
        ...
        for c_id, cut in enumerate(cuts):
                p = cut.GetEff() / 100
                n = cut.GetAll()
                #print(f"Cut: {cut.GetName()}, Events Passed (n): {n}, Efficiency: {p}")
...
def EventsParticleDistribution2D(mass_point,df):
    # df = df.Define("HVVCandidatePt", "H_to_VV.cand_p4.Pt()") 
...
def main(mychannel, var, hist_type):
    ...
    for i, mass_point in enumerate(mass_points):
        ...
        if hist_type =="genDistribution":
            hist, yrange=process_genDistribution_hist(mass_point, df_GenDL_Eventstree)
        if  hist_type == "EventsDistribution1D":
            hist, yrange, fname= EventsDistribution1D_hist(mass_point, df_GenbAccept_Eventstree)
        if hist_type =="EventsParticleDistribution2D": 
            hist, yrange, particle=EventsParticleDistribution2D(mass_point, df_GenDL_Eventstree)
            ROOT.gStyle.SetOptStat(0)
            hist.Draw("COLZ")
            canvas.SaveAs(f"plots/{particle}_{var}{mass_point}{mychannel}.png")
            continue
    ...
    if hist_type =="EventsDistribution1D": 
            canvas.SaveAs(f"plots/{fname}_{mychbrief}_Combine.png")
    if hist_type =="comparecut": 
...
#hist_type ="EventsParticleDistribution2D"
