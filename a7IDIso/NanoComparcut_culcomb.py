...
def RecoHWWCandidateSelection(df):
    ...
    if fiducial=="on":
        ...
        df=df.Filter("mindRGenlepFatjet>0.6", "mindR(Genlep, Fatjets)>0.6")
    
    if mychannel == "sle":
        #find reco idx matched to gen by mindR
        df = df.Define("Electron_p4", "ROOT::VecOps::RVec<LorentzVectorM> p4(Electron_pt.size()); \
                               for (int i = 0; i < Electron_pt.size(); i++) { \
                                   p4[i] = LorentzVectorM(static_cast<double>(Electron_pt[i]), \
                                                          static_cast<double>(Electron_eta[i]), \
                                                          static_cast<double>(Electron_phi[i]), \
                                                          static_cast<double>(Electron_mass[i])); \
                               } \
                               return p4;")
    ...
    if mychannel== "slmuon":
        df = df.Define("Muon_p4", "ROOT::VecOps::RVec<LorentzVectorM> p4(Muon_pt.size()); \
                               for (int i = 0; i < Muon_pt.size(); i++) { \
                                   p4[i] = LorentzVectorM(static_cast<double>(Muon_pt[i]), \
                                                          static_cast<double>(Muon_eta[i]), \
                                                          static_cast<double>(Muon_phi[i]), \
                                                          static_cast<double>(Muon_mass[i])); \
                               } \
                               return p4;")
...
def Report_cutflow_hist(canvas, mass_point, report, initial_count, genSL_count,  gen_accept_count, genb_accept_count, genWq_accept_count, skim_count, reportName="Cutflow", printOut=False):
    ...
    if ctype == "Relative":
        hist_eff.GetXaxis().SetBinLabel(2, f"GenSL{mychannelbrief}")
        hist_eff.SetBinContent(2, genSL_count/initial_count)

        hist_eff.GetXaxis().SetBinLabel(3, f"Gen_{mychannelbrief}_Lep_Accept")
...
##################################################Muon ID&&pfIso:
    ...
    # f"{mychfull}_looseId.at(RecoLepIdx) && {mychfull}_pfRelIso03_all.at(RecoLepIdx) < 0.4",  # Loose ID, Isolation < 0.4
    # f"{mychfull}_looseId.at(RecoLepIdx) && {mychfull}_pfRelIso03_all.at(RecoLepIdx) < 0.25", # Loose ID, Isolation < 0.25
    # f"{mychfull}_looseId.at(RecoLepIdx) && {mychfull}_pfRelIso03_all.at(RecoLepIdx) < 0.2",  # Loose ID, Isolation < 0.2
    # f"{mychfull}_looseId.at(RecoLepIdx) && {mychfull}_pfRelIso03_all.at(RecoLepIdx) < 0.15", # Loose ID, Isolation < 0.15

    # # Medium ID && PFRelIso cuts
    # f"{mychfull}_mediumId.at(RecoLepIdx) && {mychfull}_pfRelIso03_all.at(RecoLepIdx) < 0.4",  # Medium ID, Isolation < 0.4
    # f"{mychfull}_mediumId.at(RecoLepIdx) && {mychfull}_pfRelIso03_all.at(RecoLepIdx) < 0.25", # Medium ID, Isolation < 0.25
    # f"{mychfull}_mediumId.at(RecoLepIdx) && {mychfull}_pfRelIso03_all.at(RecoLepIdx) < 0.2",  # Medium ID, Isolation < 0.2
    # f"{mychfull}_mediumId.at(RecoLepIdx) && {mychfull}_pfRelIso03_all.at(RecoLepIdx) < 0.15", # Medium ID, Isolation < 0.15

    # # Tight ID && PFRelIso cuts
    # f"{mychfull}_tightId.at(RecoLepIdx) && {mychfull}_pfRelIso03_all.at(RecoLepIdx) < 0.4",  # Tight ID, Isolation < 0.4
    # f"{mychfull}_tightId.at(RecoLepIdx) && {mychfull}_pfRelIso03_all.at(RecoLepIdx) < 0.25", # Tight ID, Isolation < 0.25
    # f"{mychfull}_tightId.at(RecoLepIdx) && {mychfull}_pfRelIso03_all.at(RecoLepIdx) < 0.2",  # Tight ID, Isolation < 0.2
    # f"{mychfull}_tightId.at(RecoLepIdx) && {mychfull}_pfRelIso03_all.at(RecoLepIdx) < 0.15", # Tight ID, Isolation < 0.15
    # ]

    # labels = [
    # # Loose ID labels
    # "LooseID && PFRelIso03_all< 0.4",
    # "LooseID && PFRelIso03_all< 0.25",
    # "LooseID && PFRelIso03_all< 0.2",
    # "LooseID && PFRelIso03_all< 0.15",

    # # Medium ID labels
    # "MediumID && PFRelIso03_all< 0.4",
    # "MediumID && PFRelIso03_all< 0.25",
    # "MediumID && PFRelIso03_all< 0.2",
    # "MediumID && PFRelIso03_all< 0.15",

    # # Tight ID labels
    # "TightID && PFRelIso03_all< 0.4",
    # "TightID && PFRelIso03_all 0.25",
    # "TightID && PFRelIso03_all< 0.2",
