...
def Eff_RecoGenMatch_hist(mass_point,df):
    ...
    ###df=df.Filter("mindRGenlepFatjet>0.2", "")
    #df_sz=df.Filter("nElectron>0", "")
    #If RecoLepIdx is negative, then out-of-bounds exception
    df_sz=df.Filter(f"{mychfull}_mvaNoIso_WP90.at(RecoLepIdx)", "")
    ...
    h_after = df_sz.Histo1D(("h_after", f"mvaNoIso_WP90 Eff vs {var1} (M-{mass_point})", bins, var_min, var_max), f"{particl}").GetValue()
    hist_pt = h_before.Clone("h_ratio")
    #hist_pt.SetTitle(f"Has_e_Eff vs {particl}_{var} M-{mass_point}")
    #hist_pt.SetTitle(f"Has_e_Eff vs {var1} M-{mass_point}")
    hist_pt.SetTitle(f"mvaNoIso_WP90 vs {var1} M-{mass_point}")
    #hist_pt.GetYaxis().SetTitle(f"Eff (after HasRecoElectron)")
    hist_pt.GetYaxis().SetTitle(f"Relative Eff (after mvaNoIso_WP90)")
    legend.AddEntry(h_before, "Before mvaNoIso_WP90", "l")
    legend.AddEntry(h_after, "After mvaNoIso_WP90", "l")
    ...
    #variable changes. only for TEfficiency no getxaxis
    # hist_pt.SetTitle(f"Has_e_Eff vs {particl}_{var} M-{mass_point};{var}, SL{mychbrief} Channel;Eff(after HasRecoElectron)")
    #hist_pt.SetTitle(f"Has_e_Eff vs {var1} M-{mass_point};{var1}, SL{mychbrief} Channel;Eff(after HasRecoElectron)")
    hist_pt.SetTitle(f"mvaNoIso_WP90 vs {var1} M-{mass_point};{var1}, SL{mychbrief} Channel;Eff(after mvaNoIso_WP90)")
    ...
    c.SaveAs(f"plots/{mychbrief}{mass_point}{particl}{var1}_Eff_MtNano.png")

    return hist_pt,yrange

def Eff2D_hist(mass_point,df):
    df = df.Define("HVVCandidatept", "H_to_VV.cand_p4.Pt()")
    df = df.Define("HVVCandidateeta", "H_to_VV.cand_p4.eta()")
    #print(df.Display(["HVVCandidatept", "HVVCandidateeta"]).Print())

    pt_min = 0
    pt_max=3000
    pt_bins = int((pt_max - pt_min)/50)  
    eta_min = -5
    eta_max = 5
    eta_bins = 20

    hist2D = ROOT.TH2D("HVV_2D", f"HVV (pT, eta) distribution,M{mass_point}", pt_bins, pt_min, pt_max, eta_bins, eta_min, eta_max)

    #or model method
    data = df.AsNumpy(["HVVCandidatept", "HVVCandidateeta"])
    for pt, eta in zip(data["HVVCandidatept"], data["HVVCandidateeta"]):
        hist2D.Fill(pt, eta)

    canvas = ROOT.TCanvas("c", "2D Efficiency", 800, 600)
    ROOT.gStyle.SetPalette(ROOT.kBird)
    hist2D.Draw("COLZ")
    hist2D.SetTitle(f"HVV (pT, eta) distribution,M{mass_point};HVVCandidate pT(GeV);HVVCandidate eta")

    canvas.SaveAs(f"plots/Distr2D{mass_point}.png")

    return 0

def Eff2DRelaeff_hist(mass_point,df):
    df = df.Define("HVVCandidatept", "H_to_VV.cand_p4.Pt()")
    df = df.Define("HVVCandidateeta", "H_to_VV.cand_p4.eta()")
    #print(df.Display(["HVVCandidatept", "HVVCandidateeta"]).Print())
    df = df.Define("GenLep0pt", "GenLep0_p4.pt()")
    ...
    pt_min = 0
    pt_max=2000 #3000 for HVV only
    pt_bins = int((pt_max - pt_min)/50)

    #change variable:
    # var2d1="HVVCandidatept"
    # var2d2="HVVCandidateeta"
    # eta_min = -5
    # eta_max = 5
    # eta_bins = 20
    ...
    legend = ROOT.TLegend(0.7, 0.8, 0.9, 0.9)
    h_denom = ROOT.TH2D("denominator", "Denominator", pt_bins, pt_min, pt_max, eta_bins, eta_min, eta_max)
    h_num = ROOT.TH2D("numerator", "Numerator", pt_bins, pt_min, pt_max, eta_bins, eta_min, eta_max)

    # df.Foreach(lambda pt, eta: h_denom.Fill(pt, eta), ["HVVCandidatept", "HVVCandidateeta"])
    # df_sz.Foreach(lambda pt, eta: h_num.Fill(pt, eta), ["HVVCandidatept", "HVVCandidateeta"])
    
    # def fill_denom(pt, eta):
    #     h_denom.Fill(pt, eta)
    # def fill_num(pt, eta):
    #     h_num.Fill(pt, eta)
    # df.Foreach(fill_denom, ["HVVCandidatept", "HVVCandidateeta"])
    # df_sz.Foreach(fill_num, ["HVVCandidatept", "HVVCandidateeta"])

    data = df.AsNumpy([f"{var2d1}", f"{var2d2}"])
    for pt, eta in zip(data[f"{var2d1}"], data[f"{var2d2}"]):
        h_denom.Fill(pt, eta)
    data_sz = df_sz.AsNumpy([f"{var2d1}", f"{var2d2}"])
    for pt, eta in zip(data_sz[f"{var2d1}"], data_sz[f"{var2d2}"]):
        h_num.Fill(pt, eta)

    efficiency_2D = ROOT.TEfficiency(h_num, h_denom)

    #change variables
    canvas = ROOT.TCanvas("c", "2D Efficiency", 800, 600)
    ROOT.gStyle.SetPalette(ROOT.kBird)
    efficiency_2D.Draw("COLZ")
    #efficiency_2D.SetTitle(f"HasRecoE Eff VS (pT,Eta)of HVV,M{mass_point};HVVCandidate pT(GeV);HVVCandidate eta")
    #efficiency_2D.SetTitle(f"HasRecoE Eff VS (GenElePt,mindR),M{mass_point};GenEle Pt(GeV);mindR(GenEle, Fatjets)")
    #efficiency_2D.SetTitle(f"HasRecoE Eff VS (GenElePt,dR),M{mass_point};GenEle Pt(GeV);dR(GenEle, Hadronic W)")
    #efficiency_2D.SetTitle(f"HasRecoE Eff VS (GenEleEta,mindR),M{mass_point};GenEle Eta;mindR(GenEle, Fatjets)")
    efficiency_2D.SetTitle(f"HasRecoE Eff VS (GenEleEnergy,mindR),M{mass_point};GenEle Energy(GeV);mindR(GenEle, Fatjets)")

    legend = ROOT.TLegend(0.7, 0.8, 0.9, 0.9)
    legend.AddEntry(h_num, "After HasRecoE", "l")
    legend.AddEntry(h_denom, "Before HasRecoE", "l")
    legend.Draw()

    canvas.SaveAs(f"plots/eff2D{mass_point}.png")
...
def main(mychannel, var, hist_type):
    ...
    for i, mass_point in enumerate(mass_points):
        ...
        if  hist_type == "Eff_RecoGenMatch":
            ...
            hist, yrange= Eff_RecoGenMatch_hist(mass_point, df)
            continue
        if  hist_type == "Eff2D_hist":
            Eff2D_hist(mass_point, df_GenWqAccept_Eventstree)
            continue
        if  hist_type == "Eff2DRelaeff_hist":
            Eff2DRelaeff_hist(mass_point, df_GenWqAccept_Eventstree)
            continue
...
#hist_type = "Eff2D_hist"
#hist_type = "Eff2DRelaeff_hist"
